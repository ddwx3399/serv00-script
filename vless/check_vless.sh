...
EOL
        ...
        jq --arg port "$port" '.port = ($port | tonumber)' ~/domains/$USER.ct8.pl/vless/config.json > ~/domains/$USER.ct8.pl/vless/config_tmp.json && mv ~/domains/$USER.ct8.pl/vless/config_tmp.json ~/domains/$USER.ct8.pl/vless/config.json
...
deploy_vless() {
    ...
    npm install
    # 启动vless项目
    ~/.npm-global/bin/pm2 start ~/domains/$USER.ct8.pl/vless/app.js --name vless
    # 保存pm2进程状态
